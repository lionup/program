...
log using log_match_ind3_occ2, text replace
...
* some industry, occupation, and education category do not exist so just give them value 0
  ...
  qui foreach i of numlist `ind_num' { 
        forvalues j = 1/43 {
          forvalues m = 1/6 {
            count if ind7090 == `i' & docc1990 == `j' & educ6c == `m'
            if r(N) == 0 {
                set obs `=_N + 1'
                replace date = `x'
  ...
  if `x' != 197601{
	append using ../result/ind3_occ2.dta
  }
  sort year qtr month date ind7090 docc1990 educ6c
  saveold ../result/ind3_occ2.dta, replace
