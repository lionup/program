*Program: match_ind3_occ3.do
*Date 5/April/2012

*************************************************************************************************
* THIS PROGRAM GENERATES EMPLOYMENT SHARE BY EDUCATION, 3-DIGIT INDUSTRY, and 3-DIGIT OCCUPATION* 
*************************************************************************************************

set more 1

log using log_match_ind3_occ3, text replace
...
while `x' <= 201108 {
  ...
  * keep only the records at least 16 years old, employed, and with no missing value on education, ind7090, occ1990
  ...
  drop if educ6c == . | ind7090 == . | occ1990 == .

  sort ind7090 occ1990 educ6c

  * sum weight for each category  
  replace fweight = 0 if fweight == .
  egen double em_share = total(fweight), by (ind7090 occ1990 educ6c)
  ...
  sort ind7090 occ1990 educ6c
  quietly by ind7090 occ1990 educ6c:  gen duplic = cond(_N==1,0,_n)
  drop if duplic > 1
  keep ind7090 occ1990 educ6c em_share
  gen date = `x'
...
* some industry, occupation, and education category do not exist so just give them value 0
  ...
  qui foreach i of local ind_num { 	
        foreach j of local occ_num { 
          forvalues m = 1/6 {
            count if ind7090 == `i' & occ1990 == `j' & educ6c == `m'
            if r(N) == 0 {
                set obs `=_N + 1'
                replace date = `x'
  ...
  order year qtr month date ind7090 occ1990 educ6c em_share, first

  if `x' != 197601{
	append using ../result/ind3_occ3.dta
  }
  sort year qtr month date ind7090 occ1990 educ6c
  saveold ../result/ind3_occ3.dta, replace
