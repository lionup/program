*Program: match_indv_occ3.do
*Date 5/April/2012

*********************************************************************************************
* THIS PROGRAM GENERATES EMPLOYMENT SHARE BY EDUCATION, BEA INDUSTRY, and 3-DIGIT OCCUPATION* 
*********************************************************************************************

set more 1

log using log_match_indv_occ3, text replace
...
while `x' <= 201108 {
  ...
  * keep only the records at least 16 years old, employed, and with no missing value on education, vind7090, occ1990
  ...
  drop if educ6c == . | vind7090 == . | occ1990 == .

  sort vind7090 occ1990 educ6c

  * sum weight for each category  
  replace fweight = 0 if fweight == .
  egen double em_share = total(fweight), by (vind7090 occ1990 educ6c)
  ...
  sort vind7090 occ1990 educ6c
  quietly by vind7090 occ1990 educ6c:  gen duplic = cond(_N==1,0,_n)
  drop if duplic > 1
  keep vind7090 occ1990 educ6c em_share
  gen date = `x'
  ...
  * some industry, occupation, and education category do not exist so just give them value 0
  ...
  qui forvalues i = 1/21 {
        foreach j of numlist `occ_num' {
          forvalues m = 1/6 {
            count if vind7090 == `i' & occ1990 == `j' & educ6c == `m'
            if r(N) == 0 {
                set obs `=_N + 1'
                replace date = `x'
  ...
  order year qtr month date vind7090 occ1990 educ6c em_share, first

  if `x' != 197601{
	append using ../result/indv_occ3.dta
  }
  sort year qtr month date vind7090 occ1990 educ6c
  saveold ../result/indv_occ3.dta, replace
