*Program: match_ind2_occ1.do
*Date 5/April/2012

*************************************************************************************************
* THIS PROGRAM GENERATES EMPLOYMENT SHARE BY EDUCATION, 2-DIGIT INDUSTRY, and 1-DIGIT OCCUPATION* 
*************************************************************************************************

set more 1

log using log_match_ind2_occ1, text replace
...
while `x' <= 201108 {
  ...
  * keep only the records at least 16 years old, employed, and with no missing value on education, ind7090, occ1990
  ...
  drop if educ6c == . | dind7090 == . | mocc1990 == .

  sort dind7090 mocc1990 educ6c

  * sum weight for each category  
  replace fweight = 0 if fweight == .
  egen double em_share = total(fweight), by (dind7090 mocc1990 educ6c)
  ...
  sort dind7090 mocc1990 educ6c
  quietly by dind7090 mocc1990 educ6c:  gen duplic = cond(_N==1,0,_n)
  drop if duplic > 1
  keep dind7090 mocc1990 educ6c em_share
  gen date = `x'
...
* some industry, occupation, and education category do not exist so just give them value 0
  ...
  qui foreach i of numlist `ind_num' { 
        forvalues j = 1/13 {
          forvalues m = 1/6 {
            count if dind7090 == `i' & mocc1990 == `j' & educ6c == `m'
            if r(N) == 0 {
                set obs `=_N + 1'
                replace date = `x'
  ...
  order year qtr month date dind7090 mocc1990 educ6c em_share, first

  if `x' != 197601{
	append using ../result/ind2_occ1.dta
  }
  sort year qtr month date dind7090 mocc1990 educ6c
  saveold ../result/ind2_occ1.dta, replace
