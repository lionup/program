*Program: match_ind1_occ2.do
*Date 5/April/2012

*************************************************************************************************
* THIS PROGRAM GENERATES EMPLOYMENT SHARE BY EDUCATION, 1-DIGIT INDUSTRY, and 2-DIGIT OCCUPATION* 
*************************************************************************************************

set more 1

log using log_match_ind1_occ2, text replace
...
while `x' <= 201108 {
  ...
  * keep only the records at least 16 years old, employed, and with no missing value on education, ind7090, occ1990
  ...
  drop if educ6c == . | mind7090 == . | docc1990 == .

  sort mind7090 docc1990 educ6c

  * sum weight for each category  
  replace fweight = 0 if fweight == .
  egen double em_share = total(fweight), by (mind7090 docc1990 educ6c)
  ...
  sort mind7090 docc1990 educ6c
  quietly by mind7090 docc1990 educ6c:  gen duplic = cond(_N==1,0,_n)
  drop if duplic > 1
  keep mind7090 docc1990 educ6c em_share
  gen date = `x'
  ...
  * some industry, occupation, and education category do not exist so just give them value 0
  qui forvalues i = 1/22 {
        forvalues j = 1/43 {
          forvalues m = 1/6 {
            count if mind7090 == `i' & docc1990 == `j' & educ6c == `m'
            if r(N) == 0 {
                set obs `=_N + 1'
                replace date = `x'
  ...
  order year qtr month date mind7090 docc1990 educ6c em_share, first

  if `x' != 197601{
	append using ../result/ind1_occ2.dta
  }
  sort year qtr month date mind7090 docc1990 educ6c
  saveold ../result/ind1_occ2.dta, replace
